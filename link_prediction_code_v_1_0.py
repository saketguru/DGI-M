...
def compute_roc_ap_aupr_score(train_pos_arr,train_neg_arr,test_pos_arr,test_neg_arr,embedding0,embedding1,n_jobs=28,dot_product=False,edge_representation=None,grid_search=True,percent_edges = -1):
    if(percent_edges != -1):
    ...
    #rep0 = embedding0[all_edges[:,0]]
    #rep1 = embedding1[all_edges[:,1]]
    ...
    train_ap=average_precision_score(train_y, train_prob_preds)
    test_ap=average_precision_score(test_y, test_prob_preds)
    train_aupr = aupr(train_y,train_prob_preds)
    test_aupr = aupr(test_y,test_prob_preds)
    #print("train_auc",train_auc,"test_auc",test_auc)

    return train_auc,test_auc,train_ap,test_ap,train_aupr,test_aupr,best_c,best_penalty