...
def load_embeddings(embeddings_file, word2vec_format, graph, indices):
    ...
    if word2vec_format:
        ...
        tmp_matrix = numpy.squeeze(numpy.load(embeddings_file), axis=0)
        print (tmp_matrix.shape)
        print (indices)
        features_matrix = numpy.squeeze(numpy.load(embeddings_file), axis=0)[indices]
...
def calc_metrics(normalized, dataset, algorithm, num_shuffles, all_results, best_classifiers, writetofile,
                 embedding_params, emb_size, output_dir_and_classifier_name):
    columns = ["Algorithm", "Dataset", "Train %", "Normalized Embeddings", "Micro-F1", "Macro-F1", "Accuracy",
               "Num of Shuffles", "Embedding Size"]
    ...
    for train_percent in sorted(all_results.keys()):
        ...
        if embedding_params != None:
            ...
            print ("{},{},{},{},{},{},{},{},{},{}".format(algorithm, dataset, normalized, train_percent,
                                                          avg_score["micro"], avg_score["macro"], avg_score["accuracy"],
                                                          num_shuffles, emb_size, embedding_params_string))
        else:
            print ("{},{},{},{},{},{},{},{},{},{}".format(algorithm, dataset, normalized, train_percent,
                                                          avg_score["micro"], avg_score["macro"], avg_score["accuracy"],
                                                          num_shuffles, emb_size))

    if writetofile:
        output_file_name = os.path.join(
            output_dir_and_classifier_name + "_" + dataset + "_" + "classi_results" + "_" + algorithm + embedd_params_file_name + ".csv")
...
def logistic_regression_classification(X_train, X_test, y_train_, y_train, y_test_, y_test, grid_search,
...
def svc_classification(X_train, X_test, y_train_, y_train, y_test_, y_test, grid_search, multi_label_binarizer,
                       test_kernel):
    ...
    if test_kernel == 'eigenpro':
        ...
        X_train, y_train_eig, X_test, y_test_eig = X_train.astype('float32'), y_train_eig.astype(
            'float32'), X_test.astype('float32'), y_test_eig.astype('float32')
        ...
        eval_f = lambda s: init_train_Gaussian(s, X_train[:n_train], y_train_eig[:n_train], X_val, y_val, epochs=[10],
                                               n_class=n_class, mem_gb=12, device=device, print_flag=print_flag)
        try:
            best_val, best_param = line_search(eval_f, 1, 50,
                                               print_flag=print_flag)  # search kernel bandwidth in [1, 50]
        ...
        res, prob = model.fit(X_train[:n_train], y_train_eig[:n_train], X_val, y_val, epochs=[1, 2, 5, 10, 20],
                              mem_gb=12, print_flag=print_flag)
    ...
    if grid_search:
        ...
        svc_classifer = GridSearchCV(svc_classifer, param_grid=parameters, cv=5, scoring='f1_micro', n_jobs=-1,
                                     verbose=0)
...
def classify(emb="", network="", training_percents=training_percents_default, dataset="", writetofile=True,
             algorithm="", adj_matrix_name="network", label_matrix_name="group", num_shuffles=10, word2vec_format=True,
             embedding_params=None, classifier="LR", test_kernel="linear", grid_search=True, output_dir="./"):
    # 0. Validate inputs parameters:
    validate_inputs(emb, network, training_percents, dataset, algorithm, writetofile)

    # 1. Load labels
    mat, A, graph, labels_matrix, labels_count, multi_label_binarizer, indices = load_graph(network, adj_matrix_name,
                                                                                            label_matrix_name)
    ...
    for features_matrix_tuple in feature_matrices:
        ...
        for train_percent in training_percents:
            ...
            for x in range(num_shuffles):
                ...
                X, y = shuf
                X_train, X_test, y_train_, y_train, y_test_, y_test = get_dataset_for_classification(X, y,
                                                                                                     train_percent)

                clf = None
                results = {}
                if classifier == 'LR':
                    clf, results = logistic_regression_classification(X_train, X_test, y_train_, y_train, y_test_,
                                                                      y_test, grid_search, multi_label_binarizer)

                elif classifier == 'SVM':
                    clf, results = svc_classification(X_train, X_test, y_train_, y_train, y_test_, y_test, grid_search,
                                                      multi_label_binarizer, test_kernel)
        ...
        calc_metrics(normalized, dataset, algorithm, num_shuffles, all_results, best_classifiers, writetofile,
                     embedding_params, X.shape[1], output_dir + classifer_name_for_file)
...
def main():
    ...
    parser.add_argument("--network", type=str, required=True,
                        help='The path and name of the .mat file containing the adjacency matrix and node labels of the input network')
    parser.add_argument("--dataset", type=str, required=True, help='The name of your dataset (used for output)')
    parser.add_argument("--algorithm", type=str, required=True,
                        help='The name of the algorithm used to generate the embeddings (used for output)')
    ...
    parser.add_argument("--writetofile", action="store_true",
                        help='If the flag is set, then the results are written to a file')
    parser.add_argument("--adj_matrix_name", default='network',
                        help='The name of the adjacency matrix inside the .mat file')
    parser.add_argument("--word2vec_format", action="store_true",
                        help='If the flag is set, then genisim is used to load the embeddings')
    ...
    parser.add_argument("--training_percents", default=training_percents_default, type=arg_as_list,
                        help='List of split "percents" for training and test sets (default is [0.1, 0.5, 0.9]')
    parser.add_argument("--label_matrix_name", default='group',
                        help='The name of the labels matrix inside the .mat file')
    parser.add_argument("--classifier", default="LR", choices=["LR", "SVM", "EigenPro"],
                        help='Classifier to be used; Choose from "LR" or "SVM"')
    parser.add_argument("--test_kernel", default="linear", choices=["linear", "rbf", "eigenpro"],
    ...
    classify(emb=args.emb, network=args.network, dataset=args.dataset, algorithm=args.algorithm,
             num_shuffles=args.num_shuffles, writetofile=args.writetofile, classifier=args.classifier,
             adj_matrix_name=args.adj_matrix_name, word2vec_format=args.word2vec_format,
             embedding_params=args.embedding_params, training_percents=args.training_percents,
             test_kernel=args.test_kernel, label_matrix_name=args.label_matrix_name, grid_search=True)
