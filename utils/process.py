...
def my_load_data(dataset_str):  # {'pubmed', 'citeseer', 'cora'}
    """Load data."""
    names = ['graph']
    objects = []
    for i in range(len(names)):
    ...
    graph = objects[0]
    # print(len(nx.from_scipy_sparse_matrix(graph).nodes()))
    adj = nx.adjacency_matrix(nx.from_scipy_sparse_matrix(graph))
